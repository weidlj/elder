...
def convert_audio_to_16k_pcm(file_bytes):
...
def render_elder_page():
    ...
    uploaded = st.file_uploader(" ", type=['wav', 'mp3', 'm4a', 'aac', 'ogg', 'webm'], label_visibility="collapsed")
    if uploaded:
        if not st.session_state.last_file or uploaded.name != st.session_state.last_file:
            st.session_state.last_file = uploaded.name
            file_bytes = uploaded.read()
            wav_data = convert_audio_to_16k_pcm(file_bytes)
            if not wav_data:
                st.session_state.last_reply = "音频格式错误或ffmpeg未安装"
                return
            asr = XF_ASR(
